<template>
    <div id="loader" class="w-full h-full fixed left-0 top-0 z-[1000] flex justify-center items-center">
        <div class="flex flex-col items-center gap-6">
            <div v-if="!settingStore.isExperienceReady" class="text-primary font-bold text-6xl text-center">{{ Math.round(settingStore.loaderProgress * 100).toString() }}<span class="text-2xl">%</span></div>
